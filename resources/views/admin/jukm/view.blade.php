                ...
                <div id="tambahdata" class="modal fade" tabindex="-1" role="dialog"  aria-hidden="true" style="display: none;">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <form action="/adm/sektorusaha/store" method="POST" id="formStore">
                            @csrf
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="field-3" name="nama" placeholder="Sektor Usaha UMKM">
                                        </div>
                                    </div>
                                </div>
                            </div>
					...
					<table class="table border-0 custom-table comman-table datatable mb-0">
						<thead>
							<tr>
								...
								<th style="text-align:center;">Sektor Usaha</th>
								...
								<td class="text-end">
									<div class="dropdown dropdown-action">
										...
										<div class="dropdown-menu dropdown-menu-end">
                                            @if ($d->status == 0)
                                            <a type="button" data-id="{{ Crypt::encrypt($d->id_jukm) }}" data-bs-toggle="modal" data-bs-target="#statusdata" class="dropdown-item stat"><i class="fa-solid fa-check m-r-5"></i> Aktifkan</a>
                                            @else
                                            <a type="button" data-id="{{ Crypt::encrypt($d->id_jukm) }}" data-bs-toggle="modal" data-bs-target="#statusdata" class="dropdown-item stat"><i class="fa-solid fa-ban m-r-5"></i> Nonaktifkan</a>
                                            @endif
											<a type="button" href="" data-id="{{ Crypt::encrypt($d->id_jukm) }}" data-bs-toggle="modal" data-bs-target="#editdata" class="dropdown-item edit"><i class="fa-solid fa-pen-to-square m-r-5"></i> Edit</a>
                                            <a type="button" href="" data-id="{{ Crypt::encrypt($d->id_jukm) }}" data-bs-toggle="modal" data-bs-target="#hapusdata" class="dropdown-item hapus"><i class="fa fa-trash-alt m-r-5"></i> Hapus</a>
                                            </div>
										</div>
									</div>
								</td>
                ...
                <div id="editdata" class="modal fade" tabindex="-1" role="dialog">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <form action="/adm/sektorusaha/update" method="POST" id="formStore">
                            @csrf
...
<script>
$('.edit').click(function(){
    var id_jukm = $(this).attr('data-id');
    $.ajax({
             type: 'POST',
             url: '/adm/sektorusaha/edit',
             cache: false,
             data: {
                 _token: "{{ csrf_token() }}",
                 id_jukm: id_jukm
